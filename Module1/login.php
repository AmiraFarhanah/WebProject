            ...
            <?php
                ...
                if(isset($_POST["submit"])){
                    ...
                    if($usergroup=="Normal User"){
                        ...
                        $result=mysqli_query($con, "SELECT * FROM administrator WHERE Username='$username' AND Usergroup='$usergroup' AND Password ='$password'")or die("Select Error");
                        $row=mysqli_fetch_assoc($result);
                        if(mysqli_num_rows($result)!=0){
                                $_SESSION['valid']= $row['Username'];
                                $_SESSION['name']= $row['Name'];
                        ...
                        $result=mysqli_query($con, "SELECT * FROM food_vendor WHERE Username='$username' AND Usergroup='$usergroup' AND Password ='$password' AND Status= 'Approved'")or die("Select Error");
                        $row=mysqli_fetch_assoc($result);
                        if(mysqli_num_rows($result)!=0){
                                $_SESSION['valid']= $row['Username'];
                                $_SESSION['name']= $row['Name'];
                                $_SESSION['password']= $row['Password'];
                                $_SESSION['address']= $row['_Address'];
                                $_SESSION['phonenumber']= $row['Phonenumber'];
                                $_SESSION['email']=$row['Email'];
                                $_SESSION['usergroup']= $row['Usergroup'];
                                $_SESSION['id']=$row['ID'];
                            ...
                            echo "<div class='message'>
                            <p>Wrong Username or Password or Registration still in pending</p>
                            </div> <br>";
                            echo"<a href='login.php'><button class='btn'>Go Back</button>";
